...
// Helper function to get student data with calculated fields
async function getStudentData(user) {
    return new Promise((resolve, reject) => {
        db.get('SELECT SUM(lessonsCompleted) as completed, SUM(totalLessons) as total FROM progress WHERE userId = ?', [user.id], (err, row) => {
            if (err) {
                console.error('Error fetching progress:', err);
                return resolve(user);
            }

            const totalLessons = row.total || 0;
            ...
            const engagement = engagementScore > 70 ? 'Alto' : engagementScore > 40 ? 'Médio' : 'Baixo';
            const avatarColor = user.id.startsWith('admin') ? '#dc3545' : (user.id.startsWith('demo') ? '#ffc107' : '#007bff');
            const initials = (user.name || '').split(' ').map(n => n[0] ? n[0].toUpperCase() : '').join('').substring(0, 2) || '??';
            
            const lastLoginDate = user.lastLogin ? new Date(user.lastLogin) : null;
            const lastAccess = lastLoginDate ? lastLoginDate.toLocaleDateString('pt-BR') : 'Nunca';
            const lastAccessTime = lastLoginDate ? lastLoginDate.toLocaleTimeString('pt-BR') : '';
            const isOnline = lastLoginDate && (Date.now() - lastLoginDate.getTime()) < 5 * 60 * 1000;
            const onlineStatus = isOnline ? 'Online' : 'Offline';
            
            const statusLabel = user.status === 'ACTIVE' ? 'Ativo' : user.status === 'INACTIVE' ? 'Inativo' : 'Concluído';
...
// GET /api/students - Get all students with calculated metrics
app.get('/api/students', authenticateToken, async (req, res) => {
    if (req.user.role !== 'ADMIN') {
        return res.status(403).json({ error: 'Acesso negado' });
    }

    db.all('SELECT * FROM users WHERE role = "STUDENT" OR role = "student"', async (err, users) => {
        if (err) {
        ...
        let studentsWithData = await Promise.all(users.map(getStudentData));
        
        // Mock data if no students are found (for initial testing)
        if (studentsWithData.length === 0) {
             studentsWithData.push({
                ...
                lastAccessTime: new Date().toLocaleTimeString('pt-BR'),
                onlineStatus: 'Online'
            });
            studentsWithData.push({
                id: 'mock-2',
...
app.post('/api/students/:id/reactivate', authenticateToken, (req, res) => {
    if (req.user.role !== 'ADMIN') {
        return res.status(403).json({ error: 'Acesso negado' });
    }
    const { id } = req.params;
    db.run('UPDATE users SET status = "ACTIVE" WHERE id = ?', [id], function (err) {
        if (err) {
            console.error('Database error reactivating student:', err);
...
app.post('/api/students/:id/certificate', authenticateToken, (req, res) => {
    if (req.user.role !== 'ADMIN') {
        return res.status(403).json({ error: 'Acesso negado' });
    }
    const { id } = req.params;
    // In a real application, this would generate and send a certificate.
    ...
    db.get('SELECT name, email FROM users WHERE id = ?', [id], (err, user) => {
        ...
        const subject = 'Seu Certificado de Conclusão da gmp Academy';
        const text = `Parabéns, ${user.name}! Seu certificado de conclusão do curso está pronto.`;
        
        // Simulate sending email
        sendRealEmail(user.email, subject, text).then(result => {
            if (result.success) {
                res.json({ success: true, message: 'Certificado enviado com sucesso' });
            } else {
                res.status(500).json({ error: 'Erro ao enviar certificado por email' });
            }
        });
    });
});
...
app.post('/api/students/:id/message', authenticateToken, (req, res) => {
    if (req.user.role !== 'ADMIN') {
        return res.status(403).json({ error: 'Acesso negado' });
    }
    const { id } = req.params;
    ...
    db.get('SELECT name, email FROM users WHERE id = ?', [id], (err, user) => {
        ...
        const fullSubject = `Mensagem da Administração - ${subject}`;
        
        // Simulate sending email
        sendRealEmail(user.email, fullSubject, body).then(result => {
            if (result.success) {
                res.json({ success: true, message: 'Mensagem enviada com sucesso' });
            } else {
                res.status(500).json({ error: 'Erro ao enviar mensagem por email' });
            }
        });
    });
});

// GET /api/students/export - Export students list
app.get('/api/students/export', authenticateToken, (req, res) => {
    if (req.user.role !== 'ADMIN') {
        return res.status(403).json({ error: 'Acesso negado' });
    }
    
    db.all('SELECT id, name, email, status, lastLogin FROM users WHERE role = "STUDENT" OR role = "student"', (err, users) => {
        if (err) {
            console.error('Database error exporting students:', err);
            return res.status(500).json({ error: 'Erro ao exportar lista de alunos' });
        }
        
        // Simple CSV generation
        const header = 'ID,Nome,Email,Status,Último Acesso\n';
        const csv = users.map(u => `${u.id},"${u.name}","${u.email}",${u.status},${u.lastLogin}`).join('\n');
...
// POST /api/students/import - Import users (stub)
...
app.post('/api/students/import', authenticateToken, upload.single('file'), (req, res) => {
    ...
    // In a real application, process the CSV/XLSX file here and insert into DB
    // For now, we'll simulate a successful import
    console.log(`File received: ${req.file.originalname}`);
    
    // Clean up the uploaded file (important!)
    const fs = require('fs');
    fs.unlink(req.file.path, (err) => {
        if (err) console.error('Error deleting uploaded file:', err);
    });

    // Simulate importing 5 users
    res.json({ success: true, imported: 5, message: 'Importação simulada com sucesso' });
});

// POST /api/students/segments - Create segmentation (stub)
app.post('/api/students/segments', authenticateToken, (req, res) => {
    if (req.user.role !== 'ADMIN') {
        return res.status(403).json({ error: 'Acesso negado' });
    }
    ...
    if (!name || !criteria) {
        return res.status(400).json({ error: 'Nome e critério são obrigatórios' });
    }
    
    const segmentId = generateId('seg');
    db.run(
        'INSERT INTO segments (id, name, criteria) VALUES (?, ?, ?)',
        [segmentId, name, criteria],
        function (err) {
            if (err) {
                console.error('Database error creating segment:', err);
                return res.status(500).json({ error: 'Erro ao criar segmentação' });
            }
            res.json({ success: true, message: 'Segmentação criada com sucesso', segmentId });
        }
    );
});

// Fallback for /api/students to handle old structure if needed
app.get('/api/admin/students', authenticateToken, (req, res) => {
    res.redirect(307, '/api/students');
});

// Fallback for /api/admin/students/stats to handle old structure if needed
