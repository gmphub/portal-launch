...
app.get('/api/admin/students/stats', authenticateToken, (req, res) => {
    console.log('Stats API called by user:', req.user.userId);
    ...
    Promise.all(queries.map(query =>
        ...
        console.log('Final stats response:', statsData);
        res.json({
            success: true,
            stats: statsData
        });
...
app.post('/api/admin/students/import', authenticateToken, upload.single('file'), async (req, res) => {
  try {
    ...
    for (const line of lines) {
      ...
      if (!name || !email || !password) {
        failed++;
        errors.push(`Linha inválida: faltam dados - ${line}`);
        continue;
      }
      ...
      try {
        ...
        if (existingUser) {
          failed++;
          errors.push(`Email já cadastrado: ${email}`);
          continue;
        }
        ...
        await new Promise((resolve, reject) => {
          db.run(
            'INSERT INTO users (id, name, email, password, role, status, isEmailVerified) VALUES (?, ?, ?, ?, ?, ?, ?)',
            [userId, name, email, hashedPassword, 'STUDENT', 'ACTIVE', 0],
            function(err) {
              if (err) reject(err);
              else resolve(this);
            }
        ...
        failed++;
        errors.push(`Erro ao importar ${email}: ${error.message}`);
    ...
    res.json({ 
      ...
      message: `Importação concluída: ${imported} sucesso, ${failed} falhas`,
      errors: errors.slice(0, 10) // Return only first 10 errors
    });
  } catch (error) {
    console.error('Import error:', error);
    res.status(500).json({ success: false, error: 'Erro ao importar usuários: ' + error.message });
  }
});
...
app.post('/api/admin/students/:id/certificate', authenticateToken, async (req, res) => {
  try {
    const studentId = req.params.id;
    
    // Get student information
    db.get('SELECT * FROM users WHERE id = ?', [studentId], async (err, student) => {
      if (err) return res.status(500).json({ success: false, error: 'Erro ao buscar aluno' });
      if (!student) return res.status(404).json({ success: false, error: 'Aluno não encontrado' });
      
      // Check if student has completed courses
      db.get(
        'SELECT COUNT(*) as completedCourses FROM progress WHERE userId = ? AND progress = 100',
        [studentId],
        async (err, progressData) => {
          if (err) return res.status(500).json({ success: false, error: 'Erro ao verificar progresso' });
          
          if (!progressData || progressData.completedCourses === 0) {
            return res.status(400).json({ 
              success: false, 
              error: 'Aluno não tem cursos concluídos para receber certificado' 
            });
          }
          
          // Get certificate template or create default one
          db.get('SELECT * FROM certificate_templates LIMIT 1', [], async (err, template) => {
            const certificateId = `CERT${Date.now()}${Math.random().toString(36).substr(2, 9)}`;
            const templateId = template?.id || 'DEFAULT';
            
            // Create certificate record
            db.run(
              'INSERT INTO certificates (id, userName, userEmail, course, templateId, issuedAt) VALUES (?, ?, ?, ?, ?, ?)',
              [certificateId, student.name, student.email, 'Curso Completo', templateId, new Date().toISOString()],
              async (err) => {
                if (err) {
                  console.error('Error creating certificate:', err);
                  return res.status(500).json({ success: false, error: 'Erro ao criar certificado' });
                }
                
                // Send certificate email
                const emailSubject = 'Parabéns! Seu Certificado está Pronto! 🎓';
                const emailBody = `
                  <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                    <h2 style="color: #4CAF50;">Parabéns, ${student.name}!</h2>
                    <p>É com grande satisfação que informamos que seu certificado está pronto!</p>
                    <p>Você concluiu com sucesso <strong>${progressData.completedCourses}</strong> curso(s) em nossa plataforma.</p>
                    <div style="background-color: #f0f0f0; padding: 20px; margin: 20px 0; border-radius: 10px; text-align: center;">
                      <h3 style="color: #2196F3;">Certificado de Conclusão</h3>
                      <p><strong>ID:</strong> ${certificateId}</p>
                      <p><strong>Emitido em:</strong> ${new Date().toLocaleDateString('pt-BR')}</p>
                    </div>
                    <p>Parabéns pela sua dedicação e esforço!</p>
                    <p>Continue aprendendo e evoluindo conosco.</p>
                    <br>
                    <p>Atenciosamente,<br>Equipe GMP Academy</p>
                  </div>
                `;
                ...
                const emailResult = await sendRealEmail(
                  student.email,
                  emailSubject,
                  `Parabéns ${student.name}! Seu certificado (ID: ${certificateId}) foi emitido com sucesso.`,
                  emailBody
                );
                ...
                if (emailResult.success) {
                  res.json({ 
                    success: true, 
                    message: 'Certificado criado e enviado com sucesso!',
                    certificateId: certificateId
                  });
                } else {
                  res.json({ 
                    success: true, 
                    message: 'Certificado criado, mas houve erro ao enviar email',
                    certificateId: certificateId,
...
app.post('/api/admin/students/bulk-message', authenticateToken, async (req, res) => {
  try {
    ...
    db.all('SELECT name, email FROM users WHERE role = "STUDENT"', [], async (err, students) => {
      if (err) return res.status(500).json({ success: false, error: 'Erro ao buscar alunos' });
      
      let sentCount = 0;
      let failedCount = 0;
      const emailSubject = subject || 'Mensagem Importante da GMP Academy';
      
      // Send emails to all students
      for (const student of students) {
        try {
          const emailBody = `<h3>Olá ${student.name},</h3><p>${message.replace(/\n/g, '<br>')}</p><p>Atenciosamente,<br>Equipe GMP Academy</p>`;
      ...
      res.json({ 
        ...
        message: `Mensagens enviadas: ${sentCount} de ${students.length}` 
      });
    });
  } catch (error) {
