...
document.addEventListener('DOMContentLoaded', function() {
    // Read URL parameters from dashboard navigation
    const urlParams = new URLSearchParams(window.location.search);
    const unitParam = urlParams.get('unit');
    const titleParam = urlParams.get('title');
    const lessonPathParam = urlParams.get('lessonPath');
    
    // Update current state based on URL parameters
    if (unitParam) {
        if (unitParam === 'student') {
            // Special case for student profile
            currentState.unitId = 1; // Default to first unit for student profile
            currentState.sectionName = 'Explicação';
            currentState.sectionIndex = 0;
            console.log('Set student profile mode');
        } else if (unitParam === 'trophy') {
            // Special case for achievements/trophy
            currentState.unitId = 71; // Last unit for achievements
            currentState.sectionName = 'Conversa'; // Final section
            currentState.sectionIndex = 2;
            console.log('Set trophy/achievements mode');
        } else {
            const unitId = parseInt(unitParam);
            if (unitId >= 1 && unitId <= units.length) {
                currentState.unitId = unitId;
                console.log('Set unit from URL parameter:', unitId);
            }
        }
    }
    
    // If a specific lesson path is provided, try to determine the section
    if (lessonPathParam) {
        if (lessonPathParam.includes('pronunciation')) {
            currentState.sectionName = 'Pronúncia';
            currentState.sectionIndex = 1;
        } else if (lessonPathParam.includes('conversation') || lessonPathParam.includes('conversa')) {
            currentState.sectionName = 'Conversa';
            currentState.sectionIndex = 2;
        } else {
        ...
        console.log('Set section from lesson path:', lessonPathParam, '→', currentState.sectionName);
    }
    ...
    console.log('Lesson page initialized with state:', currentState);
});
/**
...
function generateSidebarContent() {
    ...
    // Expand the current unit by default (based on URL parameter or default)
    const currentUnitElement = unitsContainer.querySelector(`[data-unit-id="${currentState.unitId}"]`);
    if (currentUnitElement) {
        currentUnitElement.classList.add('expanded');
    } else {
        // Fallback: expand the first unit
...
function createUnitElement(unit, index) {
    ...
    // Calculate progress (DISABLED locks: Show progress for all units)
    const progress = Math.random() * 100; // All units show random progress
...
function createLessonsForUnit(unit) {
    return sections.map((section, index) => {
        const isLocked = false; // DISABLED: All units unlocked for access
...
function setupEventListeners() {
    ...
    // Lesson item clicks (DISABLED lock check - all items accessible)
