...
class SecurityManager {
    ...
    initializeAuth() {
        ...
        if (session && this.isSessionValid(session)) {
            ...
            this.isAdmin = (session.user.role === 'admin' || session.user.role === 'ADMIN');
  ...
  return true;
    ...
    async validatePassword(password, hashedPassword) {
        try {
            // Fallback: senha não vazia
            const basic = !!password && password.length > 0;
            // Opcional: valida no backend se existir endpoint
            try {
    ...
    async registerUser(userData) {
        try {
            ...
            // Tenta /api/auth/register; se não existir, usa /api/auth/signup
    ...
    async requestPasswordReset(email) {
        try {
            ...
            // fallback local (gera token temporário)
    ...
    async validateResetToken(token) {
        try {
            ...
            // fallback local: estrutura básica
