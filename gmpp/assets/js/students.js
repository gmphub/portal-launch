...
class StudentsManager {
    ...
    setupSearchAndFilters() {
        ...
        // Combine all filters into one row
        ...
        // Create combined filter row
        const searchDiv = document.createElement('div');
        searchDiv.style.flex = '1 1 300px';
        searchDiv.innerHTML = `
            <input type="text" class="form-control" placeholder="Buscar alunos..." id="combinedSearchInput" style="width: 100%;">
        `;
        filterRow.appendChild(searchDiv);

        const statusDiv = document.createElement('div');
        statusDiv.style.flex = '0 0 auto';
        statusDiv.innerHTML = `
            <select class="form-control" id="combinedStatusFilter" style="min-width: 150px;">
        ...
        `;
        filterRow.appendChild(statusDiv);

        const levelDiv = document.createElement('div');
        levelDiv.style.flex = '0 0 auto';
        levelDiv.innerHTML = `
            <select class="form-control" id="combinedLevelFilter" style="min-width: 180px;">
                <option>Todos os níveis</option>
                <option>Alto engajamento</option>
                <option>Médio engajamento</option>
                <option>Baixo engajamento</option>
            </select>
        `;
        filterRow.appendChild(levelDiv);

        const filterBtn = document.createElement('button');
        filterBtn.className = 'btn-modern btn-modern-primary';
        filterBtn.innerHTML = '<i class="fas fa-filter"></i> Filtrar';
        filterBtn.onclick = () => {
            this.searchTerm = document.getElementById('combinedSearchInput').value;
            this.statusFilter = document.getElementById('combinedStatusFilter').value;
            this.levelFilter = document.getElementById('combinedLevelFilter').value;
            this.currentPage = 1;
            this.loadStudents();
        };
        filterRow.appendChild(filterBtn);

        // Replace existing filter area
        const filterContainer = document.querySelector('.card-body .d-flex.align-items-center.gap-2.mb-3');
        if (filterContainer) {
            filterContainer.parentNode.replaceChild(filterRow, filterContainer);
        }

        // Add event listeners to combined filters
        const combinedSearchInput = document.getElementById('combinedSearchInput');
        if (combinedSearchInput) {
            combinedSearchInput.addEventListener('input', debounce((e) => handleSearch(e.target.value), 500));
        }

        const combinedStatusFilter = document.getElementById('combinedStatusFilter');
        if (combinedStatusFilter) {
            combinedStatusFilter.addEventListener('change', (e) => {
        ...
        const combinedLevelFilter = document.getElementById('combinedLevelFilter');
        if (combinedLevelFilter) {
            combinedLevelFilter.addEventListener('change', (e) => {
